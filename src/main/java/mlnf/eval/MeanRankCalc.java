...
public class MeanRankCalc {
	...
	private String testSet, mlnfOut; 
	...
	public static void main(String[] args) throws IOException {
		...
		String mlnfOut = args[1];
		
		MeanRankCalc mr = new MeanRankCalc(testSet, mlnfOut);
	...
	public MeanRankCalc(String testSet, String mlnfOut) {
		super();
		this.testSet = testSet;
		this.mlnfOut = mlnfOut;
	}
	
	public double start() throws FileNotFoundException {
		
		pw = new PrintWriter(new File(mlnfOut + "/evaluation.csv"));
		
		Scanner in = new Scanner(new File(mlnfOut + "/entities.csv"));
		int entities = 0;
		...
		for(int i=m.length; i>=1; i--) {
			...
			String discovered = mlnfOut + "/ranked_" + thr + ".nt";
		...
		System.out.println("\n=== FILTERED SETTING === "+mlnfOut.substring(mlnfOut.lastIndexOf('/')+1));
		// compute mean rank
		int sum = 0, sumR = 0;
		...
		pw.println(mlnfOut + "," + mrr + "," + h1 + "," + h3 + "," + h10);
		pw.close();
	...
	public void partitionData() throws IOException {
		...
		for (int i = minThr; i <= 9; i++) {
			...
			String outA = mlnfOut + "/discovered_" + thrA + ".nt";
			String outB = mlnfOut + "/discovered_" + thrB + ".nt";
			String ranked = mlnfOut + "/ranked_" + thrA + ".nt";
		...
		FileUtils.copyFile(new File(mlnfOut + "/discovered_1.0.nt"), new File(mlnfOut + "/ranked_1.0.nt"));
